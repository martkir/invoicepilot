---
name: mercury-ui-light
description: The light-theme twin of the house UI skill. Mercury design tokens (light, electric-blue accent, Geist, strictly flat, pill interactives) fused with the anti-slop build process - brief inference, three dials, layout discipline, AI-tell bans, redesign audit, and a mandatory pre-flight check. Use for every design task on a light surface, whether building new screens and pages or redesigning existing ones.
...
> The light-theme twin of `mercury-ui`. Identical to it except for the palette in Section 1 and the theme lock in Section 4.12, which locks light instead of dark. If a rule changes in one file it changes in the other; nothing else here is allowed to diverge.
...
components:
  ...
  input-text:
    backgroundColor: "rgba(251, 252, 253, 0.6)" # paper, with transparency
    color: "{colors.ink}"
    rounded: "{rounded.pill}"
    padding: "{spacing.sm} {spacing.lg}"
...
* **Any project:** `:root { --canvas: #eef0f6; --paper: #fbfcfd; ... --space-lg: 24px; --r-pill: 9999px; --font: ...; }` in a single tokens file. Components reference `var(--paper)`, never `#fbfcfd`.
...
* **`{colors.primary-bright}` is not spent on a light surface.** `{colors.on-primary}` over it is 3.4:1, so it is never a background for a label, and it measures 3.0:1 on `{colors.canvas}` and 2.7:1 on `{colors.canvas-deep}`, so it fails as a non-text mark too. A tint lighter than the accent has nowhere to sit on a light page. It stays in the palette for parity with the dark twin; icons, graphic marks, meter fills and status dots take `{colors.primary}` (4.1:1 on `{colors.canvas}`, 4.6:1 on `{colors.paper}`) or `{colors.primary-deep}`. The primary button darkens to `{colors.primary-deep}` on hover.
...
* **No theme toggle at all** in this system. Section 4.12 is light-only, so the sun/moon switch question does not arise.
...
* **Logo walls use real SVG logos** - Simple Icons (`https://cdn.simpleicons.org/{slug}/1e1e2a`) or devicon. Not text wordmarks in a row. For invented brands, generate a simple monogram mark.
* **LOGO-ONLY rule:** a logo wall contains logos and nothing else. No category label under each logo.
* All imagery must sit legibly on `{colors.canvas}`. Ensure single-color logos render in `{colors.ink}` or `{colors.ink-soft}`.
...
### 4.12 Theme Lock
**This system is light. There is no dark mode.** `scheme: light` in Section 1 is a brand decision, not a default.

* All surfaces derive from `{colors.canvas}` / `{colors.canvas-deep}` / `{colors.paper}`. A dark section sandwiched between light ones reads as a copy-paste accident.
* Do not implement a `prefers-color-scheme: dark` branch, a theme toggle, or `dark:` variant pairs. There is one theme.
* Contrast targets still apply in full: WCAG AA minimum for body, AAA target for display copy. `{colors.ink}` on `{colors.canvas}` clears both at 14.4:1.
* Section-level variation within the light family is fine (`{colors.canvas-deep}` beside `{colors.canvas}`). Flipping to a dark background mid-page is broken.
* No pure `#ffffff` and no pure `#000000` as surfaces. Section 1 already avoids both: `{colors.paper}` is the lightest surface in the system and it is not white.
* Hover darkens on a light surface. A control on `{colors.canvas}` lifts to `{colors.paper}`; one that is already `{colors.paper}` darkens to `{colors.canvas-deep}`. There is no lighter step above `{colors.paper}` to reach for.
...
- [ ] Single theme - light throughout, no dark section, no theme toggle, no `dark:` pairs?
...
* Dual light/dark mode protocol, `prefers-color-scheme` branching, theme toggles - the system is light only.
...
* The sun/moon theme-toggle alternative - there is no toggle, the system is light only.
